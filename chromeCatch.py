...
# 'valipl-vip.cp31.ott.cibntv.net/' in ajaxUrl and 
            if 'm3u8' in ajaxUrl:
                print('videoDataUrl:'+ajaxUrl[:80])
                m3u8Link = ajaxUrl
            elif 'sub.ykimg.com/' in ajaxUrl and '.ass' in ajaxUrl:
                print('assUrl:'+ajaxUrl)
                assLink = ajaxUrl
        except Exception: #异常不处理
            pass
...
def m3u8ToMp4():
    ...
    cmdStr ='.\tools\ffmpeg.exe -protocol_whitelist "file,http,https,rtp,udp,tcp,tls"  -i .\hy\csy\11.m3u8 .\hy\csy\11.mp4'
    os.system(cmdStr)
    pass
